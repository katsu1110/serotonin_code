...
%     disp('Synchronization index analysis --------------------')
%     % LFP split by SI (drug x SI)
%     % delta SI in base vs drug
%     % median SI in base vs drug
%     
%     ts = linspace(-0.1, stmdur, 1000*(stmdur+0.2));
%     lent = length(ts);
%     sicols = [0.1 0.1 0.1; 0.4 0.4 0.4; 1 0.2 0; 1 0.6 0]; 
%     figure(j + 1);
%     
%     for s = 1:ss
%         % data extraction
%         lfp = zeros(lenses, lent, 2, 2);
%         medsi = zeros(lenses, 2, 2);
%         for i = 1:lenses
%             for d = 1:2
%                 % synchronization index
%                 si = datast{i}.cond(d).si{stmidx(i, s)};
%                 medsi(i, d, 1) = nanmedian(si);
%                 medsi(i, d, 2) = nanstd(si);
%                 
%                 % median split
%                 idx = sortval(si, 2);            
%                 for m = 1:2                    
%                     % mean LFP traces
%                     lfp_me = nanmean(datast{i}.cond(d).lfpfull{stmidx(i, s)}(idx{m}, :), 1);
%                     lfp(i, :, d, m) = interp1(datast{i}.ts, lfp_me, ts);
%                 end
%              end
%         end
%             
...
%                 
%                 % LFP traces
%                 subplot(lena*ndrug, 3*ss, 1 + 3*(s-1) + 3*ss*(k-1) + ndrug*3*ss*(a-1))
%                 c = 1;
%                 for d = 1:2
%                     for m = 1:2
%                         me = squeeze(nanmean(lfp(cond, :, d, m), 1));
%                         sem = squeeze(nanstd(lfp(cond, :, d, m), [], 1))/sqrt(sum(cond));
%                         fill_between(ts, me-sem, me+sem, sicols(c, :), 0.5)
%                         hold on;
%                         plot(ts, me, '-', 'color', sicols(c, :))
%                         hold on;
%                         c = c + 1;
%                     end
%                 end
...
%                 
%                 % SD SI
%                 subplot(lena*ndrug, 3*ss, 2 + 3*(s-1) + 3*ss*(k-1) + ndrug*3*ss*(a-1))
%                 sd1 = squeeze(medsi(cond, 1, 2));
%                 sd2 = squeeze(medsi(cond, 2, 2));
%                 unity_scatter(sd1, sd2)
%                 if a==1 && k==1
%                     title('SD SI')
%                 elseif a==lena && k==ndrug
%                     xlabel(pairnames{k, 1})
%                     ylabel(pairnames{k, 2})
%                 end
%                 
%                 % median SI
%                 subplot(lena*ndrug, 3*ss, 3 + 3*(s-1) + 3*ss*(k-1) + ndrug*3*ss*(a-1))
%                 med1 = squeeze(medsi(cond, 1, 1));
%                 med2 = squeeze(medsi(cond, 2, 1));
%                 unity_scatter(med1, med2)     
%                 if a==1 && k==1
%                     title('median SI')
%                 elseif a==lena && k==ndrug
%                     xlabel(pairnames{k, 1})
...
%         end
%     end
%     set(gcf, 'Name', 'Synchronization index', 'NumberTitle', 'off')
%     j = j + 1;   
% end

%%
% % validation of fano factor ============================
% if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'ff'))==1
%     disp('fano factor analysis ---------------------------------')
%     figure(j + 1);
%     % data extraction
%     ffmat = zeros(lenses, 2, 2);
%     for i = 1:lenses
%         for d = 1:2
%             ffmat(i, :, d) = [nanmean(datast{i}.cond(d).ncff.nc), ...
%                 nanmean(datast{i}.cond(d).ncff.ff)];
%             mat0 = datast{i}.cond(1).lfprel.mat{1};
%             mat2 = datast{i}.cond(2).lfprel.mat{1};
%         end
%     end
%     
%     % visualization
...
%             end
%             for f = 1:2
%                 subplot(lena, 2*ndrug, f + ndrug*(k - 1) + 2*ndrug*(a-1))
%                 try
%                     unity_scatter(squeeze(ncff(cond, f, 1)), squeeze(ncff(cond, f, 2)))
%                 catch
%                 end
%                 if f==1
%                     if a==1
...
%         end
%     end
%     set(gcf, 'Name', 'noise corr. fano factor', 'NumberTitle', 'off')
%     j = j + 1;   
% end
...
if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'glm'))==1
    if strcmp(splittype, 'drug') && strcmp(datast{1}.stm.param, 'rc')
        disp('GLM analysis -------------------------')
        varnames = {'FR', 'intercept', 'drug', 'PS', 'dPS', 'drug x PS', 'drug x dPS', ...
            'LFP raw', 'ddLFP', 'theta pow', 'alpha pow', 'beta pow', 'gamma pow', 'MU'};
        lenp = length(varnames) - 1;
        ve = nan(lenses, lenp-1);
        w = nan(lenses, lenp-1);
        for i = 1:lenses
            % x and Y
            mat0 = datast{i}.cond(1).lfprel.mat{1};
            ...
            X = [[zeros(ntr0, 1); ones(ntr2, 1)], ... % base or drug
                ...
                [mat0(:, 5); mat2(:, 5)], ... % LFP
                [mat0(:, 6); mat2(:, 6)], ... % ddLFP
                [mat0(:, 7:end-2); mat2(:, 7:end-2)], ...
                [mat0(:, 2); mat2(:, 2)]]; % MU

            % fit GLM stepwise
            X = [ones(ntr0+ntr2, 1), zscore(X)];
            base = nanmean(mat0(:, 1), 1);       
            Y = log(y) - log(base);
            for k = 1:lenp-1
%                 c = @(x) cost(x, y, base, X(:, 1:k+1));
%                 beta = fminsearch(c, x0(1:k+1), options);
%                 ypred = mymodel(beta, base, X(:, 1:k+1));

                % model prediction
                beta = glmfit(X(:, 1:k+1), Y, 'normal', 'link', 'identity', 'constant', 'off');
                ypred = glmval(beta, X(:, 1:k+1), 'identity', 'constant', 'off');
                ypred = base*exp(ypred);

    %             beta = glmfit(X(:, 1:k), y, 'normal', 'link', 'identity');
    %             ypred = glmval(beta, X(:, 1:k), 'identity');

                % variance explained
                ve(i, k) = varexp(y, ypred);
            end 
        ...
        for a = 1:lena
            for k = 1:ndrug
               ...
               me = nanmean(w(cond, :), 1);
               sem = nanstd(w(cond, :), [], 1)/sqrt(sum(cond));
               for p = 1:lenp-1
                   [~, pval] = ttest(w(cond, p));
                   if pval < 0.05/lenp
                        barcol = 'r';
               ...
               if k == ndrug
                   if a == lena
                       set(gca, 'XTick', 1:lenp-1, 'XTickLabel', varnames(3:end))
                   end
                   xtickangle(45)
               end
               ...
               me = nanmean(ve(cond, :), 1);
               sem = nanstd(ve(cond, :), [], 1)/sqrt(sum(cond));
               for p = 1:lenp-1
                   barcol = 'k';
                   if p > 1
                       [~, pval] = ttest(ve(cond, p-1), ve(cond, p));
                       if pval < 0.05/lenp
                            barcol = 'r';
                       end
                       if me(p-1) > me(p) 
                           bar(p, me(p-1), 'FaceColor', barcol, 'EdgeColor', barcol)
                           hold on;
           ...
           for p = 1:lenp-1
               ...
               [~, pval] = ttest2(w(cond0, p), w(cond2, p));
               histogram(w(cond0, p), 'FaceColor', 'k')
               hold on;
               histogram(w(cond2, p), 'FaceColor', 'r')
               hold on;
               if a == 1
                   title(varnames{2+p})
               end        
    ...
    for s = 1:ss
        ...
        for i = 1:lenses
            for d = 1:2
                ...
                stas(i, :, d) = sta_temp;
%                 maxv = max(sta_temp(-0.05 < sta_t & sta_t < 0));
                [amp(i, d), zerot(i, d)] = min(sta_temp(sta_analysiswnd));
%                 amp(i, d) = maxv - amp(i, d); 
%                 amp(i, d) = abs(amp(i, d));
...
%                 for j = 1:2
%                     me = squeeze(nanmean(pow(cond, :, j), 1));
%                     sem = squeeze(nanstd(pow(cond, :, j), [], 1))/sqrt(sum(cond));
%                     fill_between(freq', me-sem, me+sem, cols(j, :), 0.5)
%                     hold on;
%                     plot(freq, me, 'color', cols(j, :))
%                     set(gca, 'box', 'off', 'tickdir', 'out')
%                     if k==1 && a==1
...
%                 end
                ...
                for b = 1:lenb
                    ...
                    stats_pairs{b, k} = [squeeze(nanmean(pow(cond, frange, 1), 2)), ...
                        squeeze(nanmean(pow(cond, frange, 2), 2))];
                end
            end
%             % plot significance            
%             for b = 1:lenb
%                 % paired test
%                 if ndrug==1
%                     stats = pair_tests(stats_pairs{b, 1});
%                 else
%                     stats = pair_tests(stats_pairs{b, 1}, stats_pairs{b, 2});
%                 end
%                 
%                 % plot (cond1)
%                 subplot(lena*ndrug, 4*ss, 3 + 4*(s - 1) + ndrug*4*ss*(a-1))
%                 if stats.pair(1).signrank.p < 0.05/lenb
%                    hold on;
%                    h = plot(bandrange{b}, yl(2)*[1 1], '-', 'color', ...
%                        0.5*[1 1 1], 'linewidth', 2);
%                    h.Color(4) = 0.4;
%                 end
%                 
%                 % plot (cond2, if any)
...
%                        hold on;
%                        h = plot(bandrange{b}, (yl(1) + 0.95*(yl(2)-yl(1)))*[1 1], '-', 'color', ...
%                            [1 0 0], 'linewidth', 2);
%                        h.Color(4) = 0.4;
%                     end
%                 end
