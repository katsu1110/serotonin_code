function para = pair_stmLFP(ex0, ex2, lfpfield, thin, analysis)
%%
...
if ismember(1, contains(analysis, 'all')) || ismember(1, contains(analysis, 'mat'))
    ...
    for d = 1:2
        ...
        for i = 1:lenv            
            para.cond(d).mat{i} = nan(para.cond(d).ntr(i), collab);
            ...
            for n = 1:para.cond(d).ntr(i)
                ...
                % mean pupil size
                stpos = para.cond(d).trials{i}(n).Eye_preprocess.stpos;
                enpos = para.cond(d).trials{i}(n).Eye_preprocess.enpos;
                para.cond(d).mat{i}(n, 3) = nanmean(para.cond(d).trials{i}(n).Eye_preprocess.ps...
                    (end-round((enpos-stpos)/4)+1:end));
                
                % pupil size derivative
                para.cond(d).mat{i}(n, 4) = nanmean(para.cond(d).trials{i}(n).Eye_preprocess.dps...
                    (end-round((enpos-stpos)/4)+1:end));
