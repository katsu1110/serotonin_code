    ...
    for d = 1:2
        ...
        for i = 1:lenv            
            ...
            for n = 1:para.cond(d).ntr(i)
                ...
                stpos = para.cond(d).trials{i}(n).Eye_prepro.stpos;
                enpos = para.cond(d).trials{i}(n).Eye_prepro.enpos;
                ps_temp = nanmean([para.cond(d).trials{i}(n).Eye_prepro.psR(stpos:enpos); ...
                    para.cond(d).trials{i}(n).Eye_prepro.psL(stpos:enpos)], 1);
                para.cond(d).mat{i}(n, 3) = nanmean(ps_temp(end-round((enpos-stpos)/psf)+1:end));
                
                % pupil size derivative
                ps_temp = nanmean([para.cond(d).trials{i}(n).Eye_prepro.dpsR(stpos:enpos); ...
                    para.cond(d).trials{i}(n).Eye_prepro.dpsL(stpos:enpos)], 1);
