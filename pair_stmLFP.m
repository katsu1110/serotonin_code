...
%             % stft
%             [~, para.cond(d).spectrogram.f{i}, para.cond(d).spectrogram.t{i}, p] = ...
%                 spectrogram_frange(para.cond(d).lfpfull{i}, 95, fs, [0 100]);
%             para.cond(d).spectrogram.S{i} = nanmean(p, 3);
        end   
    end
end
...
% compute spectrogram (v: trial x signal)
[nsc, nov, nff] = stff_params(length(v), overlap);
ntr = size(v, 1);
for n = 1:ntr
    [s_temp, f_temp, t, p_temp] = spectrogram(v(n, :), nsc, nov, nff, fs);
    if n==1
        outrange = f_temp >= frange(1) & f_temp <= frange(2);
        f = f_temp(outrange)';
        s = nan(length(f), length(t), ntr);
        p = s;
    end
    s(:, :, n) = s_temp(outrange, :);
    p(:, :, n) = p_temp(outrange, :);
...
function newmat = mat_align(mat, new_t)
nr = size(mat, 1);
old_t = size(mat, 2);
newmat = nan(nr, new_t);
for r = 1:nr
    newmat(r, :) = interp1(1:old_t, mat(r,:), linspace(1, old_t, new_t));
end