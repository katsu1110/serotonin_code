...
%                 vec = filter(b1, a1, vec);
                ...
                % permutation
                for v = 1:trt
                    scm = mean(vec(~ismember(1:trt, v)));
                    scv = var(vec(~ismember(1:trt, v)));
                    plot(scm, scv, '.', 'color', cols(d, :))
                    hold on;
                end
                hold on;
            end
...
%% 
% firing rate as a function of trial (autocorrelation) ======================
if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'autocorr'))==1
    figure(j +1);
    for s = 1:ss
        for i = 1:lenses
            subplot(nrow, ncol, i)
            for d = 1:2
                vec = datast{i}.cond(d).lfprel.mat{end}(:,1);
                [cor, ~, lag] = corrlag(vec, vec, round(length(vec)/2), 0, 'Pearson', 0);
                cor(lag==0) = [];
                lag(lag==0) = [];
                plot(lag, cor, '-', 'color', cols(d, :))
                hold on;
            end
...
if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'tufit'))==1
    ...
    for s = 1:ss
        for i = 1:lenses
            ...
            % plot
            for d = 1:2
                errorbar(datast{i}.cond(1).tuning.fitparam.val.or, ...
                    datast{i}.cond(d).tuning.fitparam.val.mn, ...
                    datast{i}.cond(d).tuning.fitparam.val.sem, ...
                    '-', 'color', cols(d, :), 'capsize', 0)
                hold on; 
            end
...
%                 title([animals{lists(i,3)+1} ', ' pairnames{lists(i,2)+1, 2}])
    ...
    set(gcf, 'Name', 'Tuning curve', 'NumberTitle', 'off')
    j = j + 1;
end
